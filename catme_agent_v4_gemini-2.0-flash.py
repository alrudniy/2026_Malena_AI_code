...
#GEMINI_MODEL = "gemini-2.5-pro"     # More advanced, but may have stricter rate limits and costsS
...
def classify_comment(comment, row_idx, total):
    ...
    # ── Print comment being sent to API ────────────────────────────────────
    print(f"\n  ┌─ [Row {row_idx}/{total}] COMMENT SENT TO API " + "─" * 30)
    print(f"  │  {comment_str}")
    print(f"  └─" + "─" * 50)
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            print(f"  ┌─ [Row {row_idx}/{total}] RAW API RESPONSE " + "─" * 31)
            print(f"  │  {text.strip()}")
            print(f"  └─" + "─" * 50)
