...
def _call_chat(messages, timeout=120):
    """Call /api/chat and return raw content string, or None on failure."""
    payload = {
        "model":    OLLAMA_MODEL,
        "messages": messages,
        "stream":   False,
        # "format": "json" intentionally omitted — Qwen3's <think> blocks are not
        # valid JSON, causing Ollama to return empty content when format is enforced.
        # JSON compliance is enforced via system prompt + few-shot examples instead.
        "options": {
            "temperature": 0.7,   # Qwen3 non-thinking mode recommended
            "top_p":       0.8,
            "top_k":       20,
            "num_predict": 1024,  # increased: 300 was causing truncation (done_reason: length)
        }
    }
...
def _call_generate(prompt, timeout=120):
    """Fallback: call /api/generate (single string prompt) when /api/chat returns empty."""
...
def classify_comment(comment, row_idx, total):
    """Classify a single comment via Ollama, with /api/generate fallback for empty responses."""
    ...
    # Build /api/chat messages
    messages = [{"role": "system", "content": SYSTEM_PROMPT}]
    messages.extend(FEW_SHOT_EXAMPLES)
    messages.append({"role": "user", "content": f'Classify: "{comment_str}" /no_think'})

    # Build /api/generate prompt (fallback) — flatten system + few-shot + query into one string
    generate_prompt = SYSTEM_PROMPT + "\n\n"
    for m in FEW_SHOT_EXAMPLES:
        role = "User" if m["role"] == "user" else "Assistant"
        generate_prompt += f"{role}: {m['content']}\n"
    generate_prompt += f'User: Classify: "{comment_str}" /no_think\nAssistant:'

    last_error = ""
    for attempt in range(MAX_RETRIES):
        try:
            # ── Primary: /api/chat ──────────────────────────────────────
            text, data = _call_chat(messages)

            # ── Debug log always ────────────────────────────────────────
            print(f"  ┌─ [Row {row_idx}/{total}] RAW RESPONSE (chat, attempt {attempt+1}) " + "─" * 8)
            print(f"  │  content    : {repr(text[:300])}")
            print(f"  │  done_reason: {data.get('done_reason')}  done: {data.get('done')}")
            print(f"  └─" + "─" * 50)

            # ── Fallback: /api/generate if chat returned empty ──────────
            if not text.strip():
                print(f"  [Row {row_idx}/{total}] Chat returned empty — trying /api/generate fallback...")
                text, data2 = _call_generate(generate_prompt)
                print(f"  ┌─ [Row {row_idx}/{total}] RAW RESPONSE (generate fallback) " + "─" * 14)
                print(f"  │  response   : {repr(text[:300])}")
                print(f"  │  done_reason: {data2.get('done_reason')}")
                print(f"  └─" + "─" * 50)

            if not text.strip():
                last_error = "Empty response from both /api/chat and /api/generate"
                print(f"  [Row {row_idx}/{total}] Both endpoints returned empty (attempt {attempt+1})")
