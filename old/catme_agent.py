...
# GEMINI_MODEL = "gemini-3-pro"     # strongest available
# GEMINI_MODEL = "gemini-2.5-pro"   # strong reasoning, good price

GEMINI_URL = f"https://generativelanguage.googleapis.com/v1beta/models/{GEMINI_MODEL}:generateContent?key={GEMINI_API_KEY}"

INPUT_FILE = "CATME_DATA_Gates1_6_Fall2019_reduced_rows_cols.xlsx"
OUTPUT_FILE = "output_CATME_LLM_Classified.xlsx"

MAX_RETRIES = 3
RETRY_DELAY = 2
BATCH_DELAY = 0.15  # delay between API calls (rate limiting)
...
def classify_comment(comment, row_idx, total):
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            print(f"  [Row {row_idx}/{total}] Parse error (attempt {attempt+1}): {e}")
    ...
    return {**EMPTY_RESULT, "confidence": 0.0, "reasoning": "CLASSIFICATION FAILED after retries"}
